...
(function( $ ) {
	...
	$( window ).load(function() {
		...
		jQuery.when(
			...
			jQuery("#reference-search-form-submit").click(
				// Use BtoA to encode
				function () {
					submitSearchForm();
					return false;
			...
			jQuery(".AMCSDMineralNameLetter").click(function () {
				referenceFilterMineralNameList(jQuery(this).html())
			});

			// TODO Add filtering for valid AMCSD records
			referenceFilterMineralNameList("A")
	...
	function submitSearchForm() {
		...
		if($("#txt_general").val() && $("#txt_general").val().trim() !== '') {
			search_json[general_search] = $("#txt_general").val().trim();
		}

		// Get Author search field
		if($("#txt_author").val() && $("#txt_author").val().trim() !== '') {
			search_json[reference_author] = $("#txt_author").val().trim();
		}

		// Get Title search field
		if($("#txt_title").val() && $("#txt_title").val().trim() !== '') {
			search_json[reference_title] = $("#txt_title").val().trim();
		}

		// Get Journal search field
		if($("#txt_journal").val() && $("#txt_journal").val().trim() !== '') {
			search_json[reference_journal] = $("#txt_journal").val().trim();
		}

		// Get Year search field
		if($("#txt_year").val() && $("#txt_year").val().trim() !== '') {
			search_json[reference_year] = $("#txt_year").val().trim();
		}
...
function referenceClearMineralNameList() {
...
function referenceFilterMineralNameList(letter) {
